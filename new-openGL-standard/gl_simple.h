#ifndef __gl_simple_h_
#define __gl_simple_h_ 1
...
#define GL_CONTEXT_RELEASE_BEHAVIOR_FLUSH 0x82FC
	...
	extern PFNGLGETTEXTUREIMAGEPROC glGetTextureImage; // read pixel from texture object
	// new VertexArray
...
public:
	...
	HDC OpenGL_HDC = NULL;
	HGLRC OpenGL_RC = NULL;

	// class method
	void init_simple_OpenGL(HWND dummy);
	void create_context_from(HWND glWnd);

	// utility method
	// legacy shader
	void loadShaderSource_And_Compile(const char* path, GLenum type, GLuint* toObj);
	void createProgram_And_AttachShader(GLuint* prgPtr, GLuint* vsPtr, GLuint* gsPtr, GLuint* fsPtr);
	void getUniformLocation(GLuint* prgPtr, GLint* unfPtr, const char* unfName);

	// program pipeline shader
	void createSeparateShaderProgram(const char* path, GLenum type, GLuint* toPRG);
};
